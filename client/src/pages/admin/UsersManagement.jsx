    ...
    try {
      ...
      if (response?.data) {
        setUsers((prevUsers) =>
          prevUsers.map((user) =>
            user?._id === userId
              ? { ...user, checkedIn: response.data.checkedIn }
              : user
          )
        );
      }
  ...
  const downloadUsersPDF = () => {
    ...
    users.forEach((user) => {
      ...
      currentY +=
        Math.max(
          ...userData.map(
            (data) => doc.splitTextToSize(data, colWidth - 5).length
          )
        ) *
          lineHeight +
        5;
                    ...
                    <td>
                      <label className="toggle-switch">
                        <input
                          ...
                          onChange={() =>
                            handleCheckIn(user?._id, user?.checkedIn)
                          }
                        />
            ...
            <div className="modal-content">
              ...
              <form onSubmit={handleRegister}>
                ...
                <div className="form-group">
                  ...
                  />
                  {formErrors.nic && (
                    <span className="error-message">{formErrors.nic}</span>
                  )}
      ...
      {editingUser && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h2>Edit User</h2>
            {editFormError && (
              <div className="form-error-box">{editFormError}</div>
            )}
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                ...
                if (!editingUser.nic?.trim()) {
                  errors.nic = "NIC is required";
                  isValid = false;
                }
                if (!editingUser.phoneNumber?.trim()) {
                  errors.phoneNumber = "Phone number is required";
                  isValid = false;
                }
            ...
            >
              <div className="form-group">
                ...
                />
                {editFormErrors.name && (
                  <span className="error-message">{editFormErrors.name}</span>
                )}
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <input
                  type="text"
                  id="editNic"
                  value={editingUser.nic}
                  onChange={(e) =>
                    setEditingUser({ ...editingUser, nic: e.target.value })
                  }
                  className={editFormErrors.nic ? "input-error" : ""}
                />
                {editFormErrors.nic && (
                  <span className="error-message">{editFormErrors.nic}</span>
                )}
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  className={editFormErrors.phoneNumber ? "input-error" : ""}
                />
